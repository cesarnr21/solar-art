
import math

class velocity:
    def __init__(self, x = 0, y = 0, z = 0):
        self.x = x
        self.y = y
        self.z = z

    def __repr__(self):
        return f"velocity({self.x}, {self.y}, {self.z})"

    def __str__(self):
        return f"{self.x}i + {self.y}j + {self.z}k"

    def __getitem__(self, item):
        if item == 0:
            return self.x

        elif item == 1:
            return self.y

        elif item == 2:
            return self.z

        else:
            raise IndexError("There are only three elements in the vector")

    def __add__(self, other):
        return velocity(self.x + other.x, self.y + other.y, self.z + other.z)

    def __sub__(self, other):
        return velocity(self.x - other.x, self.y - other.y, self.z - other.z)

    def __mul__(self, other):
        if isinstance(other, velocity):  # velocity dot product
            return (self.x * other.x + self.y * other.y + self.z * other.z)

        elif isinstance(other, (int, float)):  # Scalar multiplication
            return velocity(self.x * other, self.y * other, self.z * other)

        else:
            raise TypeError("operand must be velocity, int, or float")

    def __truediv__(self, other):
        if isinstance(other, (int, float)):
            return velocity(self.x / other, self.y / other, self.z / other)

        else:
            raise TypeError("operand must be int or float")

    def get_magnitude(self):
        return math.sqrt(self.x ** 2 + self.y ** 2 + self.z ** 2)

    def normalize(self):
        magnitude = self.get_magnitude()
        return velocity(self.x / magnitude, self.y / magnitude, self.z / magnitude)
